...
export async function fetchExtensionReview(address: Address, url: string) {
    let res = await axios.get('https://tonhub-extensions-server-ogl9i.ondigitalocean.app/review/' + address.toFriendly({ testOnly: AppConfig.isTestnet, urlSafe: true }) + '/' + encodeURIComponent(url), { timeout: 5000 });
...
export async function postExtensionReview(url: string, review: ReviewInput) {
    let res = await axios.post('https://tonhub-extensions-server-ogl9i.ondigitalocean.app/review', { url, input: review });
