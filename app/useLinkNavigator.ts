...
import { ConnectedAppConnectionRemote, SendTransactionError, SignRawParams, TonConnectBridgeType } from './engine/tonconnect/types';
...
export function useLinkNavigator(
    ...
    const handler = useCallback(async (resolved: ResolvedUrl) => {
        ...
        if (resolved.type === 'tonconnect-request') {
