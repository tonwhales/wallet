...
public class DeviceCredentialsStoreModule extends ReactContextBaseJavaModule {
    ...
    static final String TAG = "DeviceCredentialsStore";
    ...
    DeviceCredentialsStoreModule(ReactApplicationContext context) {
        super(context);
        mContext = context;
    ...
    public void setValueWithKeyAsync(String value, String key, Promise promise) {
        try {
            ...
            Log.e(TAG, "Caught unexpected exception when writing to DeviceCredentialsStoreModule", e);
            promise.reject("WRITE_ERROR", "An unexpected error occurred when writing to DeviceCredentialsStoreModule", e);
        }
    }

    private void setItemImpl(String key, String value, Promise promise) {
        if (key == null) {
            promise.reject("NULL_KEY", "DeviceCredentialsStoreModule keys must not be null");
            return;
        }
        ...
        if (value == null) {
            ...
            if (success) {
                ...
                promise.reject("WRITE_ERROR", "Could not write a null value to DeviceCredentialsStoreModule");
            }
            return;
        }
        ...
        try {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.R) {
                ...
                promise.reject("BUILD_VERSION_CODES_ERROR", "There was an I/O error loading the keystore for DeviceCredentialsStoreModule");
                return;
            }
        } catch (IOException e) {
            Log.w(TAG, e);
            promise.reject("IO_ERROR", "There was an I/O error loading the keystore for DeviceCredentialsStoreModule", e);
            return;
        } catch (GeneralSecurityException e) {
            Log.w(TAG, e);
            promise.reject("ENCRYPT_ERROR", "Could not encrypt the value for DeviceCredentialsStoreModule", e);
            return;
        }
    }
    ...
    private void saveEncryptedItem(Promise promise, JSONObject encryptedItem, SharedPreferences prefs, String key) {
        ...
        if (encryptedItemString == null) { // lint warning suppressed, JSONObject#toString() may return null
            promise.reject("JSON_ERROR", "Could not JSON-encode the encrypted item for DeviceCredentialsStoreModule");
            return;
        }
        ...
        if (success) {
            ...
            promise.reject("WRITE_ERROR", "Could not write encrypted JSON to DeviceCredentialsStoreModule");
        }
    }
    ...
    public void getValueWithKeyAsync(String key, Promise promise) {
        try {
            ...
            Log.e(TAG, "Caught unexpected exception when reading from DeviceCredentialsStoreModule", e);
            promise.reject("READ_ERROR", "An unexpected error occurred when reading from DeviceCredentialsStoreModule", e);
        }
    }

    private void getItemImpl(String key, Promise promise) {
        // We use a DeviceCredentialsStoreModule-specific shared preferences file, which lets us do things like enumerate
    ...
    private void readJSONEncodedItem(String key, SharedPreferences prefs, Promise promise) {
        ...
        try {
            ...
            promise.reject("JSON_ERROR", "Could not parse the encrypted JSON item in DeviceCredentialsStoreModule");
            return;
        }
        ...
        try {
            if (AESEncrypter.NAME.equals(scheme)) {
                ...
                String message = String.format("The item for key \"%s\" in DeviceCredentialsStoreModule has an unknown encoding scheme (%s)", key, scheme);
                Log.e(TAG, message);
            ...
            Log.w(TAG, e);
            promise.reject("DECRYPT_ERROR", "Could not decrypt the item in DeviceCredentialsStoreModule", e);
        } catch (JSONException e) {
            Log.w(TAG, e);
            promise.reject("DECODE_ERROR", "Could not decode the encrypted JSON item in DeviceCredentialsStoreModule", e);
        }
    }
    ...
    public void deleteValueWithKeyAsync(String key, expo.modules.core.arguments.ReadableArguments options, expo.modules.core.Promise promise) {
        try {
            ...
            Log.e(TAG, "Caught unexpected exception when deleting from DeviceCredentialsStoreModule", e);
            promise.reject("DELETE_ERROR", "An unexpected error occurred when deleting item from DeviceCredentialsStoreModule", e);
        }
    }
    ...
    private void deleteItemImpl(String key, expo.modules.core.Promise promise) {
        ...
        if (success) {
            ...
            promise.reject("DELETE_ERROR", "Could not delete the item from DeviceCredentialsStoreModule");
        }
    }

    /**
     * We use a shared preferences file that's scoped to both the experience and DeviceCredentialsStoreModule. This
     ...
     * encrypted item.
     */
    protected static class AESEncrypter implements DeviceCredentialsStoreModule.KeyBasedEncrypter<KeyStore.SecretKeyEntry> {
