import BN from "bn.js";
import { Address, Cell } from "ton";

export function createDeploySubCell(address: Address) {
    const cell = new Cell();
    cell.bits.writeUint(1, 8); // deploy & install plugin
    cell.bits.writeInt(0, 8);
    cell.bits.writeCoins(new BN(100000000));

    console.log('[createDeploySubCell]', '1');
    let code = Cell.fromBoc(Buffer.from('', 'hex'))[0];
    console.log('[createDeploySubCell]', '2');
    let data = new Cell();
    data.bits.writeAddress(address); // wallet 
    data.bits.writeAddress(address); // beinificiary address
    data.bits.writeCoins(new BN(100000000));    // amount
    data.bits.writeUint(10 * 60, 32);               // period 10 min
    data.bits.writeUint(0, 32);               // start_time
    data.bits.writeUint(0, 32);               // timeout
    data.bits.writeUint(0, 32);               // last_payment_time
    data.bits.writeUint(0, 32);               // last_request_time
    data.bits.writeUint(0, 8);                // failed attempts
    data.bits.writeUint(0, 32);               // subscription id


    cell.withReference(code);
    cell.withReference(data);

    return cell;
}