import { memo } from "react";
import { useHiddenBanners, useMarkBannerHidden } from "../../engine/hooks/banners";
import Animated, { FadeInUp, FadeOutDown } from "react-native-reanimated";
import { Pressable } from "react-native-gesture-handler";
import { View, Text, useWindowDimensions } from "react-native";
import { Canvas, LinearGradient, Rect, vec } from "@shopify/react-native-skia";
import { Image } from "expo-image";
import { useTheme } from "../../engine/hooks";
import { Typography } from "../styles";
import { t } from "../../i18n/t";
import { useTypedNavigation } from "../../utils/useTypedNavigation";
import TonhubChangellyBannerLogo from '@assets/banners/changelly-banner-logo.svg';

const bannerId = 'tonhub-changelly-banner';

export const TonhubChangellyBanner = memo(() => {
    const hiddenBanners = useHiddenBanners();
    const markBannerHidden = useMarkBannerHidden();
    const dimentions = useWindowDimensions();
    const theme = useTheme();
    const navigation = useTypedNavigation();

    const isHidden = hiddenBanners.includes(`${bannerId}`);

    const onPress = () => {
        navigation.navigateSwap();
    }

    if (isHidden) {
        return null;
    }

    return (
        <Animated.View
            entering={FadeInUp}
            exiting={FadeOutDown}
        >
            <Pressable
                onPress={onPress}
                style={({ pressed }) => ({
                    opacity: pressed ? 0.5 : 1,
                    backgroundColor: theme.surfaceOnBg,
                    borderRadius: 20,
                    overflow: 'hidden',
                    marginHorizontal: 16, marginTop: 16
                })}
            >
                <View style={{ position: 'absolute', top: 0, bottom: 0, left: 0, right: 0, overflow: 'hidden', borderRadius: 20 }}>
                    <Canvas style={{ flexGrow: 1 }}>
                        <Rect
                            x={0} y={0}
                            width={dimentions.width - 32}
                            height={106 + 20}
                        >
                            <LinearGradient
                                start={vec(0, 0)}
                                end={vec(dimentions.width - 32, 0)}
                                positions={[0.08, 0.96]}
                                colors={['#000000', '#10D078']}
                            />
                        </Rect>
                    </Canvas>
                </View>
                <View style={{
                    flexDirection: 'row', flexGrow: 1,
                    alignItems: 'center', justifyContent: 'space-between',
                    paddingLeft: 20
                }}>
                    <View style={{ flexShrink: 1 }}>
                        <Text style={[{ color: theme.textUnchangeable }, Typography.semiBold15_20]}>
                            {t('changelly.tonhubBannerTitle')}
                        </Text>
                        <Text style={[{ color: theme.textUnchangeable, opacity: 0.8 }, Typography.regular15_20]}>
                            {t('changelly.tonhubBannerDescription')}
                        </Text>
                    </View>
                    <TonhubChangellyBannerLogo
                        width={142}
                        height={106}
                        style={{
                            marginRight: 10
                        }}
                    />

                </View>
            </Pressable>
            <Pressable
                    style={{
                        position: 'absolute',
                        top: 26, right: 26
                    }}
                    onPress={() => markBannerHidden(`${bannerId}`)}
                >
                    <Image
                        style={{
                            tintColor: theme.iconUnchangeable,
                            height: 24, width: 24
                        }}
                        source={require('@assets/ic-close.png')}
                    />
                </Pressable>
        </Animated.View>
    );
})