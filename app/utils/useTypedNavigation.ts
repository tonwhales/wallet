...
import { SimpleTransferParams } from "../fragments/secure/simple-transfer/types/SimpleTransferParams";
...
export function typedNavigateAndReplaceAll(
  src: Base,
  name: string,
  params?: any
) {
...
export class TypedNavigation {
  ...
  navigateReview(params: { type: "review" | "report"; url: string }) {
  ...
  navigateHoldersLanding(
    {
      ...
      onEnrollType,
      inviteId,
    }: { endpoint: string; onEnrollType: HoldersAppParams; inviteId?: string },
    isTestnet: boolean
  ) {
  ...
  navigateScanner(
    params: { callback: (src: string) => void },
    modal?: boolean
  ) {
    (async () => {
  ...
  navigateProductsList(params: ProductsListFragmentParams) {
    this.navigate(
      params.isLedger ? "LedgerProductsList" : "ProductsList",
      params
    );
  }
...
export function useTypedNavigation() {
  ...
  const typedNavigation = useMemo(
    () => new TypedNavigation(baseNavigation),
    [baseNavigation]
  );
