...
import { Platform, Text, View, KeyboardAvoidingView, Keyboard, Alert, Pressable, StyleProp, ViewStyle, BackHandler, LayoutAnimation } from "react-native";
...
const SimpleTransferComponent = () => {
    ...
    useEffect(() => {
        console.log(`[SimpleTransferFragment] addressDomainInput: ${addressDomainInput}`);
        LayoutAnimation.configureNext(LayoutAnimation.Presets.easeInEaseOut);
    }, [addressDomainInput]);


    useEffect(() => {
        navigation.navigateSimpleTransferAddress({
            account: (ledgerAddress ?? acc!.address).toString({ testOnly: network.isTestnet }),
            callback: (value) => setAddressDomainInputState(value)
        })
    }, []);
    return (
...
export const SimpleTransferStateFragment = fragment(SimpleTransferComponent);