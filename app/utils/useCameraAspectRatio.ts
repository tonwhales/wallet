import type { CameraRatio, CameraView } from 'expo-camera';
...
export function useCameraAspectRatio() {
	...
	const prepareRatio = async (camera: CameraView) => {
		...
		if (Platform.OS === 'android') {
			...
			for (const ratio of ratios ?? []) {
				...
				const realRatio = Number.parseInt(parts[0]) / Number.parseInt(parts[1]);
			...
			setPreviewSettings({
				...
				ratio: desiredRatio as CameraRatio,
				ready: true
			});
		}
	};
	...
	return {
		ratio: previewSettings.ratio,
		imagePadding: previewSettings.imagePadding,
		prepareRatio,
		ready: previewSettings.ready
	};
}
