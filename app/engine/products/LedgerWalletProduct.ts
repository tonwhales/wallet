...
export class LedgerWalletProduct {
    ...
    constructor(engine: Engine, address: Address) {
        ...
        this.#txsAtom = atomFamily<TransactionDescription, string>({
            ...
            default: selectorFamily({
                ...
                get: (id) => ({ get }) => {
                    ...
                    // Operation
                    let operation = resolveOperation({ body: base.body, amount: base.amount, account: base.address || address, metadata, jettonMaster: masterMetadata });
        ...
        engine.persistence.wallets.item(address).for((state) => {
            // Update pending
            ...
            // Latest transaction
            const last = engine.persistence.fullAccounts.item(address).value?.last;

            if (last) {
                let latest = this.engine.transactions.getWalletTransaction(address, last.lt.toString(10));
                ...
                for (let i = 0; i < toLoad - 1; i++) {
                    ...
                    let tx = this.engine.transactions.getWalletTransaction(address, next.lt);
        ...
        // History
        this.#history = createHistorySync(address, engine);

        // Plugins
        this.#plugins = selector({
            key: 'wallet/' + address.toFriendly({ testOnly: AppConfig.isTestnet }) + '/plugins',
            get: ({ get }) => {
                // Load wallet
                let wallet = get(engine.persistence.wallets.item(address).atom);
                if (!wallet) {
                    return { plugins: [] }
                }
    ...
    loadMoreFromStorage(nextTx: Transaction) {
        ...
        if (this.#state) {
            ...
            for (let i = 0; i < 10 - 1; i++) {
                ...
                let tx = this.engine.transactions.getWalletTransaction(this.address, next.lt);
    ...
    loadMore = (lt: string, hash: string) => {
        let tx = this.engine.transactions.getWalletTransaction(this.address, lt);
        // If found in storage 
        if (tx) {
