...
function getMasterKeyFromSeed(seed: Buffer): HDKeyState {
    const I = hmac(sha512, BITCOIN_SEED, new Uint8Array(seed));
    return {
        key: I.slice(0, 32),
...
/**
 * BIP39 standard seed derivation
 * Uses PBKDF2-HMAC-SHA512 with 2048 iterations
 */
function bip39MnemonicToSeed(mnemonic: string[], passphrase: string = ''): Buffer {
    const mnemonicString = mnemonic.join(' ');
    const salt = 'mnemonic' + passphrase;

    // BIP39 standard: PBKDF2-HMAC-SHA512, 2048 iterations, 64 byte output
    const seed = pbkdf2(sha512, mnemonicString, salt, { c: 2048, dkLen: 64 });

    return Buffer.from(seed);
}

/**
 * Derive Ethereum private key from mnemonic using BIP39/BIP32/BIP44 standard
 * This produces the same key as MetaMask and other standard wallets
 */
export async function ethereumPrivateKeyFromMnemonic(mnemonics: string[]): Promise<Uint8Array> {
    // Use standard BIP39 seed derivation (compatible with MetaMask)
    const seed = bip39MnemonicToSeed(mnemonics);
