...
import {
  useIsLedgerRoute,
  useLiquidStakingMember,
  useNetwork,
  usePrice,
  useSelectedAccount,
  useStakingPool,
  useStakingWalletConfig,
  useTheme,
} from '../../engine/hooks';
...
export const StakingCalculatorFragment = fragment(() => {
  ...
  const priceText = useMemo(() => {
    ...
    return formatCurrency(
      (
        parseFloat(fromNano(validAmount)) *
        (price ? price?.price.usd * price.price.rates[currency] : 0)
      ).toFixed(2),
      currency,
      isNeg,
    );
  ...
  return (
    <>
      ...
      <ScrollView
        ...
        contentInset={{
          bottom: keyboard.keyboardShown
            ? keyboard.keyboardHeight - safeArea.bottom
            : 0.1 /* Some weird bug on iOS */,
        ...
        <View style={{ flexGrow: 1, flexBasis: 0, alignSelf: 'stretch', flexDirection: 'column' }}>
          <View
            ...
            <View
              ...
              <Text
                ...
                {`${t('common.balance')}: `}
                <ValueComponent precision={4} value={balance} centFontStyle={{ opacity: 0.5 }} />
              </Text>
            </View>
            <ATextInput
              index={0}
              ...
              onValueChange={(newVal) => {
                ...
                if (temp.split('.')[0].length > 10) {
                  // 10 digits before dot bigger is unreallistic
                  return;
                }
                setAmount(formatted);
              }}
          ...
          {(isLiquid ? !!liquidStaking : !!pool) && validAmount !== null && (
            <StakingCalcComponent
              ...
              amount={validAmount}
              fee={
                isLiquid ? toNano(fromNano(liquidStaking!.extras.poolFee)) : pool!.params.poolFee
              }
            />
          )}
        </View>
      ...
      <KeyboardAvoidingView
        ...
        keyboardVerticalOffset={Platform.select({ ios: safeArea.top + 16, android: 0 })}>
